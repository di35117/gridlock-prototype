import pytest
from fastapi.testclient import TestClient
from main import app
from modules.websockets.manager import notifier

client = TestClient(app)

@pytest.mark.asyncio
async def test_websocket_broadcast():
    notifier.active_connections = []
    initial_count = len(notifier.active_connections)
    test_alert = {"type": "CRITICAL_ALERT", "source": "Pytest", "message": "Testing the WS Pipeline"}

    with client.websocket_connect("/api/ws/dashboard") as websocket:
        assert len(notifier.active_connections) == initial_count + 1
        
        await notifier.broadcast_alert(test_alert)
        data = websocket.receive_json()
        assert data["type"] == "CRITICAL_ALERT"

    assert len(notifier.active_connections) == initial_count